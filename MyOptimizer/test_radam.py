...
# 1. 构造一个简单的线性模型：y = w * x + b
w = jt.array([0.5]).float()
b = jt.array([0.0]).float()

# 2. 初始化 RAdam
optimizer = RAdam([w, b], lr=0.01, betas=(0.9, 0.999), eps=1e-8, weight_decay=1e-5)

# 3. 打印初始状态，确认 __init__ 正确
print("=== 初始化检查 ===")
print(f"lr:           {optimizer.lr}")
print(f"betas:        {optimizer.betas}")
print(f"eps:          {optimizer.eps}")
print(f"weight_decay: {optimizer.weight_decay}")
print(f"state:        {optimizer.state}")
print(f"buffer[0]:    {optimizer.buffer[0]}")
print(f"param_groups: {len(optimizer.param_groups)} group(s)")
print()

# 4. 跑 5 步，观察 loss 是否下降
print("=== 训练过程 ===")
x      = jt.array([2.0]).float()
target = jt.array([5.0]).float()   # 期望 w*x + b ≈ 5，即 w≈2.5, b≈0
...
for pid, s in optimizer.state.items():
    print(f"param id {pid}: step={s['step']}  "
          f"exp_avg_norm={float(s['exp_avg'].norm()):.6f}  "
          f"exp_avg_sq_norm={float(s['exp_avg_sq'].norm()):.6f}")

print()

# 6. 确认 buffer 被填充（至少第0个槽应该有值）
print("=== buffer 检查 ===")
print(f"buffer[1]: {optimizer.buffer[1]}")   # step=1 存在 index 1%10=1